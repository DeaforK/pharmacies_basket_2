...
export const ModalBallonProductItem:React.FC<Props> = ({isNotEnd,product,pharId, callbackSuccess}) => {
    ...
    return (
    // <li key={product.image} className={`flex gap-[12px] pt-[12px] `}>
    <li key={pharId} className={`flex gap-[12px] pt-[12px] `}>
        <img src="/item.png" alt="item" className={`h-[48px] w-[48px] object-contain ${(product.availableCount === 0 || product.availableCount > 2 || product.availableCount <= 2 ) && 'pb-[0px]'}`} />

        <div className={`flex flex-col border-b-[#F2F2F5] ${isNotEnd && 'border-b-[1px]'} grow pb-[12px]`}>
            <div className="flex gap-[12px] relative">
                
                <span className="max-w-[203px] grow font-normal text-[14px] leading-[18px] max-[900px]:w-[203px] block">
                    {
                        product.availableCount === 0 && <span className="text-[#EB1C4E] mb-[2px] block font-normal text-[14px]">Нет в наличии</span>
                    }
                    {
                        product.availableCount > 2 && <span className="text-[#FF9900] mb-[2px] block font-normal text-[14px]">Доступно {product.availableCount} из 10</span>
                    }
                            {
                                product.availableCount <= 2 && product.availableCount > 0 && <span onMouseEnter={ () => setHoverHint(true)} onMouseLeave={() => setHoverHint(false)}  className="w-fit items-center mb-[2px] flex gap-[6px] text-[#78808F]  font-normal text-[14px] leading-[18px]">Заканчивается
                ...
                <div className="flex  flex-col shrink-0 w-fit mt-[20px] max-w-[68px] items-end">
                    {
                        product.availableCount !== 0 &&
                        <span className="font-semibold text-right text-[14px] leading-[18px] text-[#000000] wide_text">{new_price} ₽</span>
                    }
                    <span className="text-[#78808F] text-left w-full text-[14px] font-normal leading-[18px]">× 3 шт.</span>
                </div>
            </div>
            ...
            {
                IsReplaced &&
                <div className={`${product.availability  === 'Нет в наличии'} mt-[12px] flex w-fit gap-[12px] bg-[#F7F7FA] rounded-[8px] h-[28px] pl-[12px] py-[2px] items-center`}>
                    <span className="font-normal text-[14px] leading-[18px]">Вы выбрали замену </span>
                    <div className='flex gap-[2px]'>

                        <button  className='flex justify-center  cursor-pointer items-center w-[28px] h-[28px]' onClick={() => 
                        {
                            ...
                            <img  className="h-[16px] w-[16px]" src="/edit.svg" alt="edit" />
                        </button>
                        <button className=' cursor-pointer flex justify-center items-center w-[28px] h-[28px]' onClick={() => replacementState.openModalDelete(() => setReplaced(false))}>
